...
// R. Hensel
// 11=29-20
// Convert blinking LED program into assembly code.  Use bit banding to control RCC
...
#if 0
void control_led(uint8_t state, uint32_t duration);
void delay2(uint32_t iteration);

void delay2(uint32_t iteration)
{
  uint32_t counter = iteration;
  while (counter > 0)
  {
       counter--;
  }  
...
{
    if (state == 1)
        GPIOA_ODR |= ORD5;     /* turn on   */
    else
        GPIOA_ODR &= ~ORD5;   /* turn off   */    
    delay(duration);
    return;
}
#endif
...
{    
    ...
    while(1)
    {
        ...
        //control_led(LED_ON, DELAY_DURATION);
        //control_led(LED_OFF, DELAY_DURATION);
    }
}
